...
периодом, допущениями и оценкой времени. Существенно неоднозначный запрос
проходит от двух до четырёх последовательных уточнений; ответы, пропуски и
текущий шаг восстанавливаются после обновления страницы. Рабочий run создаётся
только после завершения уточнений и явного подтверждения. Охват, период и
допущения можно исправить перед запуском; ревизии защищают от подтверждения
устаревшего варианта и повторного создания run. Из dashboard также можно
изменить автоматически созданный заголовок, перенести собственное исследование
в архив и увидеть новый результат; unread-состояние хранится отдельно для
каждого пользователя.

До подтверждения к черновику можно приложить HTTP(S)-ссылки, вставленный
текст, PDF, TXT и Markdown. Материал по умолчанию требует независимой
проверки; также доступны роли «первичный источник», «только контекст» и
«не цитировать». Администратор может переопределить автоматически подобранные
период, географию, языки, типы источников и формат отчёта или вернуть отдельные
поля в auto-режим. Подтверждённые материалы и effective-настройки замораживаются
в provenance запуска. Вложения могут передаваться настроенным внешним
AI-провайдерам, поэтому в них не следует помещать секреты или персональные
...
| `MAX_INPUT_FILE_BYTES` | Максимальный размер PDF, TXT или Markdown | `5242880` |
| `MAX_INPUT_TEXT_BYTES` | Максимальный размер извлечённого или вставленного текста | `100000` |
| `MAX_INPUT_MATERIALS` | Максимум материалов в одном черновике | `20` |
